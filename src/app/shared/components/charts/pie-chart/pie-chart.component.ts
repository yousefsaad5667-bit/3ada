...
/**
 * T021 — PieChartComponent
 * Chart.js 'pie' type. Legend position from config.legendPosition (default 'bottom').
 */
@Component({
...
export class PieChartComponent extends BaseChartDirective implements AfterViewInit {
  ...
  onExportPng(): void {
    ...
    const filename = this.config().exportFilename ?? 'pie-chart';
  ...
  private renderChart(series: ChartDataSeries[], theme: ChartTheme): void {
    ...
    // Pie chart uses first series; multiple data points map to slices
    ...
    this.initChart(this.canvasRef.nativeElement, 'pie', data, options);
  }
